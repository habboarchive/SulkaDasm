...
{
	if (args.length != 3)
		throw new Exception("Bad arguments. Usage: file.swf index code.abc");
	auto swf = SWFFile.read(cast(ubyte[])read(args[0]));
	auto index = to!uint(args[1]);
	uint count;
	foreach (ref tag; swf.tags)
		if ((tag.type == TagType.DoABC || tag.type == TagType.DoABC2) && count++ == index)
		{
			auto abc = cast(ubyte[])read(args[2]);
